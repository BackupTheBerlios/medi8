...
public class Sequence implements Visitable
{
	...
	public void addTrack(Track track)
	{
        ...
        // FIXME: remove this when a track is removed.
		track.addChangeNotifyListener(new IChangeListener () {
			public void notify(Medi8Event event) {
                if (fps == -1 && event instanceof AddEvent)
                  {
                    Clip c = ((AddEvent) event).getClip();
                    if (c instanceof FileClip)
                      fps = ((FileClip) c).getFPS();
                  }
    ...
    public void removeTrack(Track track)
    {
      tracks.remove(track);
      if (track instanceof AutomationTrack)
        automationtracks.remove(track);
      else
        videotracks.remove(track);
      updateSizes();
      // FIXME: should have a proper event type here.
      notify(new Medi8Event(this));
    }
