...
/**
 * This class generates an XML representation of a given sequence. It is used
 * for saving a sequence. The generated format is read by Medi8XMLParser;
 * changes here must be reflected there.
 */
public class XMLGeneratingVisitor
  extends Visitor
{
  ...
  {
    out.println("<select start=\"" + s.getSelectionStartTime() + "\" end=\""
                + s.getSelectionEndTime() + "\">");
    s.visitChildren(this);
    out.println("</select>");
  }
